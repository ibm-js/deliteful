...
`deliteful/ViewStack` support `delite` display infrastructure by inheriting from `delite/DisplayContainer`. For more informations, see [`delite/DisplayContainer`](/delite/docs/0.7.0/DisplayContainer.html) documentation.
...
You can set height of ``<body>`` and ``<html>`` to ``100%`` by including [`defaultapp.css`](/delite/docs/0.7.0/defaultapp.html) 
...
This widget has no specific security concern. Refer to [`delite/Widget`](/delite/docs/0.7.0/Widget.html) for general security advice on this base class that `deliteful/ViewStack` is using.
