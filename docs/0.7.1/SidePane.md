...
If `mode` is set to `"push"` or `"reveal"`, the width of the SidePane can't be changed in the markup (15em by default). However it can be changed in `SidePane_template.less` file. See [Delite themes](/delite/docs/0.7.1/themes.html).
...
This widget has no specific security concern. Refer to [`delite/Widget`](/delite/docs/0.7.1/Widget.html) for general security advice on this base class that `deliteful/SidePane` is using.
