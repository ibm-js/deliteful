...
`deliteful/ViewStack` support `delite` display infrastructure by inheriting from `delite/DisplayContainer`. For more informations, see [`delite/DisplayContainer`](/delite/docs/0.8.1/DisplayContainer.html) documentation.
...
This widget has no specific security concern. Refer to [`delite/Widget`](/delite/docs/0.8.1/Widget.html) for general security advice on this base class that `deliteful/ViewStack` is using.
