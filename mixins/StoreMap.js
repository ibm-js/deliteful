define(["dcl/dcl", "dojo/_base/lang", "./Store"], function (dcl, lang, Store) {
	...
	return dcl(Store, {
		...
		// mapAtInit: Boolean
		//		Whether the mapping occurs once when store items are loaded or on demand each time a property is
		//		accessed. This can only makes an actual difference if you are using a binding function which behavior
		//		varies over time. Default is true.
		mapAtInit: true,

		// mappedKeys: Array?
		//		Array of item keys to be considered for mapping. If null the component will be introspected to find
		//		all the properties ending with "Attr" or "Func" and provide mapping for those. Default is null.
		mappedKeys: null,
		...
		//		the render item with direct mapping. Default is false.
				...
				if (this.mapAtInit) {
					...
					(function (key) {
						Object.defineProperty(renderItem, key, {
							get: function () {
								return getvalue(self, this.__item, key, store);
							},
							set: function (value) {
								setvalue(self, this.__item, key, store, value);
							}
						});
					})(mappedKeys[i]);
				}
