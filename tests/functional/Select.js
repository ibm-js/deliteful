define(function (require) {
	...
	function checkKeyboardNavigationMultipleSelection(remote, selectId) {
		var selItemNullStr = "widget.selectedItem is null";
		// Expression executed in the browser for collecting data allowing to
		// check the state of the widget.
		var executeExpr = "return {" +
	...
	registerSuite("deliteful/Select - functional", {
		before: function () {
			return this.remote
				.get(require.toUrl("deliteful/tests/functional/Select.html"))
		...
		*/
		...
		tests: {
			...
			"keyboard navigation selectionMode = multiple": function () {
				if (!/chrome/i.test(this.remote.environmentType.browserName) ||
