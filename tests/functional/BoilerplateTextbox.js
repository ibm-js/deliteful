		...
		tests: {
			"initial conditions": function () {
				return this.remote
					...
					.execute("return document.querySelector('#dt1 .d-input-container-node')" +
						...
						assert.strictEqual(value, "dt1-label",
							"aria-labelledby set on (direct) parent of <input> nodes");
					});
			},
			...
			"shift tab to previous element": function () {
				...
				return this.remote
					...
					.execute("return document.activeElement.id;").then(function (id) {
						assert.strictEqual(id, "dt2-month-input",
							"shift-tab from first field moves to previous element");
					});
			},
