define(["intern!object",
	...
	function codeIns(action) { return action.var + ".inserted"; }
	function codeExp(action) { return action.var + ".expired";  }
	function codeRem(action) { return action.var + ".removed";  }
			...
			return remote
				...
				.then(function () {
					return remote
						...
						.then(function (actions) {
							var remotes = _.map(actions, function (action) {
									return remote
										.elementById(action.buttonId)
										.clickElement()
										.end()
										.waitForCondition(codeIns(action), TEST_TIMEOUT_MS)
										.then(function () {
											if (action.props.duration > - 1) { // if message is expirable
												return remote
