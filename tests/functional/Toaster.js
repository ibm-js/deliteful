define(function (require) {
	...
	function checkHasElement(remote, elementId) {
		return remote.findById(elementId).then(function () {
			// noop
		}, function () {
			assert.fail(elementId + " was not found");
		})
		.end();
	}
	...
	function checkHasNotElement(remote, elementId) {
		return remote.findById(elementId).then(function () {
			assert.fail(elementId + " was found");
		}, function () {
			// noop
		})
		.end();
	...
	registerSuite("Toaster tests", {
		...
		tests: {
			...
			"Check message dismissal": function () {
				var remote = this.remote;
				if (/internet explorer|edge/i.test(remote.environmentType.browserName) ||
			...
			"Check message types": function () {
				...
				function checkType(action) {
					return remote
						.findById(action.buttonId).click().end()
						.then(function () {
							return checkInsertion(remote, action.props.id).then(function () {
			...
			// Commented out, moveMouseTo() doesn't seem to work anywhere.
